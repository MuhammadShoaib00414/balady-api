...
@section('content')
    ...
    <div class="course__template">
      <div class="course__search__field justify-content-between align-items-center mb-5 wow animate__animated animate__fadeInUp">
        <p class="course__title">{{ __('admin.Certificates') }}</p>
        ...
        <table id="example" class="table" style="width:100%">
          ...
          <tbody>
            ...
            <tr class="odd">
                ...
                <td> {{$certificate->student->national_id}} </td>
                <td>
                    <div class="user__profile">
                        <p>{{$certificate->student->first_name}} {{$certificate->student->third_name}} {{$certificate->student->family_name}}</p>
                    </div>
                </td>
                <td> {{$certificate->percentage}} </td>
                <td>
                    @if($certificate->status == 'pass')
                    <button type="button" class="btn user__status active__status">{{ __('admin.Passed') }}</button>
                    @else
                    <button type="button" class="btn user__status delete__status">{{ __('admin.Fail') }}</button>
                    @endif
                  </td>
                ...
                <td>
                    <div class="btn-group dropstart option__dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <li><a href="{{route('admin.certificate.show',$certificate->id)}}" class="dropdown-item" type="button">{{ __('admin.View') }}</a></li>
                        </ul>
                    </div>
                </td>
            </tr>
