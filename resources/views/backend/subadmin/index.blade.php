...
@section('content')
    ...
    <div class="course__template">
      <div class="course__search__field justify-content-between align-items-center mb-5 wow animate__animated animate__fadeInUp">
        <p class="course__title">{{ __('admin.Sub Admin') }}</p>
        <div class="course__search__field mb-5">
            <div class="search__field">
                <div class="input-group"> </div>
            </div>
            <a type="button" href="{{route('admin.subadmin.create')}}" class="btn add__course w-100"><i class="fa fa-plus" aria-hidden="true"></i>{{ __('admin.Add Sub Admin') }}</a> </div>
    </div>
        ...
        <table id="example" class="table" style="width:100%">
          <thead>
            <tr>
                ...
                <th>{{ __('admin.Phone Number') }}</th>
                <th>{{ __('admin.Biography') }}</th>
            ...
            <tr class="odd">
                ...
                <td>
                    <div class="user__profile"> <img src="../assets/images/user-profile.png" alt="">
                        <p>{{$subadmin->first_name}} {{$subadmin->last_name}}</p>
                    </div>
                </td>
                <td> {{$subadmin->email}} </td>
                <td> {{$subadmin->phone}} </td>
                <td> {!! $subadmin->biography !!} </td>
                <td>
                    <div class="btn-group dropstart option__dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <li><a href="{{route('admin.subadmin.show',$subadmin->id)}}" class="dropdown-item" type="button">                {{ __('admin.View') }}</a></li>
                            <li><a  href="{{route('admin.subadmin.edit',$subadmin->id)}}" class="dropdown-item" type="button">               {{ __('admin.Edit') }}</a></li>
                            <li><a  href="{{route('admin.subadmin.destroy',$subadmin->id)}}" class="dropdown-item data-delete" type="button">{{ __('admin.Delete') }}</a></li>
                        </ul>
                    </div>
                </td>
            </tr>
